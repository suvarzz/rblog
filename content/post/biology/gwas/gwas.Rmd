...
output:
  blogdown::html_page:
    toc: true
summary: "Theory of Genome-Wide Association Studies (GWAS)"
---
...
* [POLYGENE script](https://sites.google.com/site/fdudbridge/software)  
* [PRSice](http://prsice.info/) | [Tutorial for PRsice](https://github.com/MareesAT/GWA_tutorial/)  
* PLINK ‐‐score  
...
[Wikipedia: Polygenic score](https://en.wikipedia.org/wiki/Polygenic_score)
...
SNP is a substitution of a single nucleotide that occures at a specific position in the genome, where each variation is present to some appreciable degree within a population (e.g. > 1%). SNPs underline differences in our susceptibility to a wide range of diseases.
...
dbSNP is a SNP database from the National Center for Biotechnology Information (NCBI). As of June 8, 2015, dbSNP listed 149,735,377 SNPs in humans.[35][36]
Kaviar[37] is a compendium of SNPs from multiple data sources including dbSNP.
...
dbSAP – single amino-acid polymorphism database for protein variation detection[38]
...
[Wiki: SNP](https://en.wikipedia.org/wiki/Single-nucleotide_polymorphism)
...
[Wiki: Genetic association](https://en.wikipedia.org/wiki/Genetic_association)  
...
LIST[48] (Local Identity and Shared Taxa) estimates the potential deleteriousness of mutations resulted from altering their protein functions. It is based on the assumption that variations observed in species closely related to human are more significant when assessing conservation compared to those in distantly related species.
SNAP2
SuSPect
PolyPhen-2
PredictSNP
...
SNPViz[49] This program provides a 3D representation of the protein affected, highlighting the amino acid change so doctors can determine pathogenicity of the mutant protein.
PROVEAN
PhyreRisk is a database which maps variants to experimental and predicted protein structures.[50]
Missense3D is a tool which provides a stereochemical report on the effect of missense variants on protein structure.[51]
...
$G_i$ refers to the ith possible genotype, out of n possibilities
Given the above framework, different software solutions for detecting SNVs vary based on how they calculate the prior probabilities {\displaystyle P(G)} P(G), the error model used to model the probabilities {\displaystyle P(D|G)} P(D|G), and the partitioning of the overall genotypes into separate sub-genotypes, whose probabilities can be individually estimated in this framework.  
...
[Wiki: SNV calling from NGS data](https://en.wikipedia.org/wiki/SNV_calling_from_NGS_data)
...
[Wiki: Hardy-Weinberg principle](https://en.wikipedia.org/wiki/Hardy-Weinberg_principle)  
...
Imagine there is a rare disease, afflicting, say, only one in many thousands of adults in a country. Imagine we suspect that being exposed to something (say, having had a particular sort of injury in childhood) makes one more likely to develop that disease in adulthood. The most informative thing to compute would be the risk ratio, RR. To do this in the ideal case, for all the adults in the population we would need to know whether they (a) had the exposure to the injury as children and (b) whether they developed the disease as adults. From this we would extract the following information: the total number of people exposed to the childhood injury, {\displaystyle N_{E},} {\displaystyle N_{E},} out of which {\displaystyle D_{E}} {\displaystyle D_{E}} developed the disease and {\displaystyle H_{E}} H_E stayed healthy; and the total number of people not exposed, {\displaystyle N_{N},} {\displaystyle N_{N},} out of which {\displaystyle D_{N}} D_N developed the disease and {\displaystyle H_{N}} {\displaystyle H_{N}} stayed healthy. Since {\displaystyle N_{E}=D_{E}+H_{E}} {\displaystyle N_{E}=D_{E}+H_{E}} and similarly for the {\displaystyle N_{N}} {\displaystyle N_{N}} numbers, we only have four independent numbers, which we can organize in a table:
...
Now the risk of developing the disease given exposure is {\displaystyle D_{E}/N_{E}} D_{{E}}/N_{{E}} (where {\displaystyle N_{E}=D_{E}+H_{E}} {\displaystyle N_{E}=D_{E}+H_{E}}), and of developing the disease given non-exposure is {\displaystyle D_{N}/N_{N}.} {\displaystyle D_{N}/N_{N}.} The risk ratio, RR, is just the ratio of the two,

{\displaystyle RR={\frac {D_{E}/N_{E}}{D_{N}/N_{N}}}\,,} {\displaystyle RR={\frac {D_{E}/N_{E}}{D_{N}/N_{N}}}\,,}
which can be rewritten as {\displaystyle RR={\frac {D_{E}N_{N}}{D_{N}N_{E}}}={\frac {D_{E}/D_{N}}{N_{E}/N_{N}}}.} {\displaystyle RR={\frac {D_{E}N_{N}}{D_{N}N_{E}}}={\frac {D_{E}/D_{N}}{N_{E}/N_{N}}}.}  
In contrast, the odds of developing the disease given exposure is {\displaystyle D_{E}/H_{E}\,,} {\displaystyle D_{E}/H_{E}\,,} and of developing the disease given non-exposure is {\displaystyle D_{N}/H_{N}\,.} {\displaystyle D_{N}/H_{N}\,.} The odds ratio, OR, is the ratio of the two,

{\displaystyle OR={\frac {D_{E}/H_{E}}{D_{N}/H_{N}}}\,,} {\displaystyle OR={\frac {D_{E}/H_{E}}{D_{N}/H_{N}}}\,,}
which can be rewritten as {\displaystyle OR={\frac {D_{E}H_{N}}{D_{N}H_{E}}}={\frac {D_{E}/D_{N}}{H_{E}/H_{N}}}.} {\displaystyle OR={\frac {D_{E}H_{N}}{D_{N}H_{E}}}={\frac {D_{E}/D_{N}}{H_{E}/H_{N}}}.}

We may already note that if the disease is rare, then OR ≈ RR. Indeed, for a rare disease, we will have {\displaystyle D_{E}\ll H_{E},} {\displaystyle D_{E}\ll H_{E},} and so {\displaystyle D_{E}+H_{E}\approx H_{E};} {\displaystyle D_{E}+H_{E}\approx H_{E};} but then {\displaystyle D_{E}/(D_{E}+H_{E})\approx D_{E}/H_{E},} {\displaystyle D_{E}/(D_{E}+H_{E})\approx D_{E}/H_{E},} in other words, for the exposed population, the risk of developing the disease is approximately equal to the odds. Analogous reasoning shows that the risk is approximately equal to the odds for the non-exposed population as well; but then the ratio of the risks, which is RR, is approximately equal to the ratio of the odds, which is OR. Or, we could just notice that the rare disease assumption says that {\displaystyle N_{E}\approx H_{E}} {\displaystyle N_{E}\approx H_{E}} and {\displaystyle N_{N}\approx H_{N},} {\displaystyle N_{N}\approx H_{N},} from which it follows that {\displaystyle N_{E}/N_{N}\approx H_{E}/H_{N},} {\displaystyle N_{E}/N_{N}\approx H_{E}/H_{N},} in other words that the denominators in the final expressions for the RR and the OR are approximately the same. The numerators are exactly the same, and so, again, we conclude that OR ≈ RR. Returning to our hypothetical study, the problem we often face is that we may not have the data to estimate these four numbers. For example, we may not have the population-wide data on who did or did not have the childhood injury.
...
The same story could be told without ever mentioning the OR, like so: as soon as we have that {\displaystyle N_{E}\approx H_{E}} N_{{E}}\approx H_{{E}} and {\displaystyle N_{N}\approx H_{N},} {\displaystyle N_{N}\approx H_{N},} then we have that {\displaystyle N_{E}/N_{N}\approx H_{E}/H_{N}.} {\displaystyle N_{E}/N_{N}\approx H_{E}/H_{N}.} Thus if, by random sampling, we manage to estimate {\displaystyle H_{E}/H_{N},} {\displaystyle H_{E}/H_{N},} then, by rare disease assumption, that will be a good estimate of {\displaystyle N_{E}/N_{N},} {\displaystyle N_{E}/N_{N},} which is all we need (besides {\displaystyle D_{E}/D_{N},} {\displaystyle D_{E}/D_{N},} which we presumably already know by studying the few cases of the disease) to compute the RR. However, it is standard in the literature to explicitly report the OR and then claim that the RR is approximately equal to it.  

[Wiki: Odds ratio](https://en.wikipedia.org/wiki/Odds_ratio)  
...
See also [Wiki: FVC ratio](https://en.wikipedia.org/wiki/FEV1/FVC_ratio) for more metrics of lung capacity.  
## Glossary
